...
let ConvertFile = (file) => {
    return new Promise((reolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            reolve({ size: fileSize, name: fileName, base64: resulss })
        }
...
let GetFileFn = (event) => {
    ...
    ConvertFile(file).then(value => {
        ...
        SubmitData.imgBase64 = value.base64 // 将base64字符串保存到数组中
        SubmitData.name = value.name// 设置图片名称
        SubmitData.size = value.size// 设置图片大小
...
let DelateImg = () => {
    ...
    //  提示用户是否被删删除掉图片的数据并且将
    // return CatPromptJS({ text: '请重新', type: 'success', timeout: "1000" })
}


let loading = ref(false);
let message = ref("正在提交中...")
...
let SubmitDaa = async () => {
    ...
    let FormDataList = [
        {
            ...
            base64: SubmitData.imgBase64,
            size: SubmitData.size,
        }
    ]
    let inputData = {
        username: SubmitData.username,
        slogin: SubmitData.slogin,
        imgUrl: SubmitData.imgUrl
    }
    ...
    // 明天完成啦
    loading.value = true
    PushModifyUsers({ FormDataList, inputData }).then(({ result }) => {
        ...
        return CatPromptJS({ text: `${data.message.length > 8 ? '修改失败' : data.message}`, type: 'error', timeout: "1000" })
    })

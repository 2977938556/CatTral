<template>
    <div>
        ...
        <div class="detail-comment-content">
            ...
            <div class="input">
                <input v-model="replay" ref="inputes" type="text" placeholder="回复当前评论" @keydown.enter="PushComment">
            </div>
                    ...
                    <li class="center-item paddings">
                        <div class="detail-pl-title-user">
                            <a href="javascript:;">
                                ...
                                <span>
                                    ...
                                    <p>{{ timeFormat(ContentData.createTime) }}</p>
                                </span>
                            </a>
                        </div>
                        <!-- 评论详情 -->
                        <div class="detail-pl-title-content">
                            ...
                            <div class="detail-pl-center" @click="addUpFN">
                                ...
                                <span>
                                    <img src="../../../../assets//image/cat-detail-pl.png" @click="focusfn" alt="b">
...
import { PushStoryLike } from '@/api/story.js'
    ...
    setup() {
        ...
        let PushComment = () => {
            ...
            PushStoryHfCommment({ content: replay.value, commentId: commentId, user_id: userId, type: "mjgs" }).then(({ result: { data } }) => {
                // 这个是添加到父级的数据
                store.commit('mjgs/AddStoryCommentRefly', { commentId, playload: data })
                // 这个是添加到主评论的回复数量中
                ContentData.value.replies.push(data._id)
                // 这个是添加到的回复数据
        ...
        // 点赞模块
        let Adddianz = () => {
            PushStoryLike({ userId, commentId: ContentData.value._id, type: 'mjgs' }).then(value => {
                store.commit('mjgs/AddStoryComment_Up', { commentId: ContentData.value._id, userId })
                const index = ContentData.value.addup.findIndex(item => item === userId);

                // 通过索引值我们进行判断是否有进行增删
                if (index < 0) {
                    ContentData.value.addup.push(userId);
                } else {
                    ContentData.value.addup.splice(index, 1);
                }
            }).catch(err => {
        ...
        return { ContentData, CommentRefyData, Adddianz, GetReplayList, inputes, timeFormat, loading, finished, focusfn, userId, replay, PushComment }
